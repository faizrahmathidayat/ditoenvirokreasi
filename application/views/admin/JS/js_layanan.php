<script>
    $('#layanan_body').summernote();
    $('#btn_tambah_layanan').on('click', function(e) {
        $('#modal_add_layanan').modal('show');
    });

    $('#datatablesSimple').on('click', '.btn_ubah_layanan', function(e) {
        var id = $(this).data('id');
        var url = '<?= base_url('dek-admin-login/tampil-layanan') ?>';
        var data = {
            'id': id
        }
        $.ajax({
            ...
            success: function(res) {
                ...
                $('.text-danger').html('');
                $('#id_layanan').val(res.id);
                $('#ubah_kategori').val(res.id_kategori_layanan);
                $('#ubah_judul').val(res.judul);
                $('#ubah_deskripsi').val(res.deskripsi);
                $('#ubah_layanan_body').summernote("code", res.layanan_body);
                $('#modal_edit_layanan').modal('show');
            },
            error: function() {
                alert('gagal memuat data, terjadi kesalahan!');
            }
        })
    });
    ...
    function btn_simpan_banner(id) {
        if (id) {
            var url = '<?= base_url('dek-admin-login/upload-banner-layanan') ?>';
    ...
    $('#form_ubah_layanan').on('submit', function(e) {
        e.preventDefault();
        var data = $(this).serialize();
        var url = '<?= base_url('dek-admin-login/ubah-layanan') ?>';
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
    ...
    $('#form_tambah_layanan').on('submit', function(e) {
        e.preventDefault();
        var data = $(this).serialize();
        var url = '<?= base_url('dek-admin-login/simpan-layanan') ?>';
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
    ...
    $('#datatablesSimple').on('click', '.btn_hapus_layanan', function(e) {
        var id = $(this).data('id');
        ...
        var url = '<?= base_url('dek-admin-login/hapus-layanan') ?>';
        Swal.fire({
    ...
    $('#datatablesSimple').on('click', '.btn_hapus_banner', function(e) {
        ...
        var url = '<?= base_url('dek-admin-login/hapus-banner-layanan') ?>';
        Swal.fire({
